...
=======
	...
	userList = cursor.fetchall()
	return [user["steamid"] for user in userList]
>>>>>>> 8e2c5b91a4a593ed825cc07f88d67a660388385e
...
def addNotFoundGamesFromSteamDB(missingGamesList, cursor):
	actionCounter = 0
	for game in missingGamesList:
		...
		m = p.findall(output)
		for tag in m:
			tags.append(removeNonAscii(tag.split(">")[1].split("<")[0]))
		joinedTags = ','.join(tags)
		...
		try:
			...
			# if we override something
			print("Error at updating" + gameName + " to Database from SteamDB \n")
			pass
		actionCounter += 1
	return actionCounter
